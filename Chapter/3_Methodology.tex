...
The raw capacitive sensing data are first imported from multiple text files and consolidated into a single data frame together with their associated metadata, including participant ID, handedness, finger label, and task information. Each raw sensor measurement is then parsed into a $27 \times 15$ capacitance matrix by extracting the timestamp, validating the expected data length, replacing missing or invalid entries with zeros, and converting all values to integers. Samples corresponding to the \texttt{PAUSE} task are subsequently removed to retain only valid finger interaction events.

To isolate the finger contact region, a recursive connected-component (blob)
detection algorithm is applied to each capacitance matrix using a predefined
intensity threshold. Samples containing no blob or multiple disconnected blobs
are discarded, ensuring that only images with a single finger contact remain.
The extracted blob is then cropped to its bounding region. A statistical analysis
of the cropped finger blobs shows that the majority of samples have widths
between 5 and 8 pixels (Figure~\ref{fig:width_distribution}) and heights between
6 and 8 pixels (Figure~\ref{fig:height_distribution}). Based on these
distributions, a fixed resolution of $8 \times 8$ pixels is adopted for all samples,
providing a standardized input representation while preserving virtually all
spatial information contained in the original finger contact regions. The resulting
preprocessed images are then serialized into a \texttt{.pkl} file for efficient
storage and reuse in subsequent experiments.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{Figure/width_distribution.png}
    \caption{Distribution of finger blob widths. Most cropped finger blobs have widths between 5 and 8 pixels.}
    \label{fig:width_distribution}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{Figure/height_distribution.png}
    \caption{Distribution of finger blob heights. Most cropped finger blobs have heights between 6 and 8 pixels.}
    \label{fig:height_distribution}
\end{figure}

\section{Spiking Neural Network based on RANC Architecture}
\label{sec:RANC-based-Spiking-Neural-Network}
...
The proposed architecture inherits the overall hierarchical design of the baseline Tea-based network. Multiple parallel Tea layers \cite{RANC} are first employed to extract local features independently, after which their outputs are concatenated and passed to a final Tea layer for feature aggregation. The classification stage is preserved from the original model, consisting of an Additive Pooling \cite{RANC} layer followed by a Softmax classifier. This design maintains the lightweight and event-driven characteristics of the Tea architecture while enabling hierarchical feature learning through multi-stage processing.

Compared with the baseline, the proposed model introduces several architectural improvements to better exploit temporal spiking information. Instead of processing a small number of parallel branches, the spike sequence is partitioned into seven overlapping temporal segments, each of which is processed by an independent Tea layer. This temporal segmentation allows each branch to capture complementary temporal patterns before feature fusion. By increasing the number of parallel Tea layers and replacing spatial partitioning with temporal segmentation, the proposed architecture provides richer temporal feature representations while remaining fully compatible with the original Tea-based classification pipeline.

\subsubsection{Spiking Feature Extraction}

Spiking feature extraction in the proposed architecture is performed using the \textit{Tea layer} \cite{RANC}, which serves as the fundamental computational unit of the network. Each \textit{Tea layer} \cite{RANC} implements a hardware-constrained spiking transformation that approximates neuromorphic computation while remaining trainable using gradient-based methods.

Let $\mathbf{x} \in \{0,1\}^N$ denote the input spike vector after encoding. The \textit{Tea layer} \cite{RANC} maintains two sets of parameters: a static weight matrix $\mathbf{W} \in \{-1,1\}^{N \times M}$ and a trainable connection matrix $\mathbf{C} \in [0,1]^{N \times M}$. During the forward pass, the connection matrix is discretized into a binary mask:
...
Each input segment is processed independently by a dedicated \textit{Tea layer} \cite{RANC}, allowing the network to extract localized spiking features in parallel. The resulting output vectors encode the spatio-temporal firing behavior of neurons and serve as high-level representations for subsequent aggregation and classification.
...
After the final spiking layer produces its output, the resultin
g spike activity must be aggregated to generate a classification decision. Unlike conventional neural networks, where outputs are typically continuous values, the proposed SNN produces binary spike events over time. Therefore, a dedicated aggregation mechanism is required to convert temporal spike patterns into class predictions.
...
\subsubsection{Architecture Parameter Configuration}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{Figure/model_architecture.drawio.png}
    \caption{RANC-based SNN Model architecture}
\end{figure}

The performance and efficiency of the proposed SNN architecture are influenced by several key design parameters, including the segment size (\textit{Elems}), the overlap between segments (\textit{Overlap}), and the number of output neurons in each Tea layer (\textit{Tea Out}). The configuration of these parameters follows the settings reported in Table~1 of the original work, which provide a suitable balance between recognition accuracy and computational cost.

The \textit{Elems} parameter determines the number of elements in each input segment. This parameter controls the receptive field of each parallel processing branch. A smaller segment size allows the model to focus on fine-grained local patterns, while a larger segment size captures broader spatial context. In this work, a moderate segment size is selected to balance these two aspects, enabling effective local feature extraction without losing important contextual information.

The \textit{Overlap} parameter specifies the number of shared elements between adjacent segments. Introducing overlap ensures that spatial continuity is preserved across segment boundaries, preventing the loss of critical information at segment edges. Additionally, overlapping segments introduce a degree of redundancy, which improves robustness to noise and variability in the capacitive fingerprint data.

The \textit{Tea Out} parameter defines the number of output neurons in each Tea layer, and thus directly affects the representational capacity of the network. Increasing the number of neurons allows the model to learn more complex feature representations, but also leads to higher computational cost. The selected value provides a trade-off between model expressiveness and efficiency, consistent with the design goals of neuromorphic systems.
...
\subsubsection{Training Mechanism of Tea Layers}
...
First, the use of \textit{overlapping segmentation} combined with parallel processing enables efficient feature extraction from localized regions of the input. By dividing the input into multiple segments and processing them simultaneously, the model reduces effective computation time while preserving important spatial information across segment boundaries. The choice of segmentation parameters, including the segment size (\textit{Elems}) and the degree of overlap (\textit{Overlap}), plays a crucial role in this design. A moderate segment size allows the model to focus on meaningful local patterns, while overlapping segments ensure continuity of spatial information and improve robustness to noise and variations in the input.

Second, the adoption of \textit{Tea layers} allows the network to operate with binary synaptic weights and discretized connections, making it suitable for neuromorphic implementations. This design reduces computational complexity and enables efficient simulation of spiking behavior while maintaining trainability. In addition, the number of output neurons in each Tea layer (\textit{Tea Out}) determines the representational capacity of the network, providing a balance between expressive power and computational cost.

Third, the hierarchical structure of the network, consisting of \textit{parallel feature extraction} followed by global aggregation, enables the model to capture both local and global patterns. This improves the discriminative capability of the network for fingerprint recognition tasks.

Finally, the use of \textit{Additive Pooling} for output aggregation provides a robust mechanism for decision making based on spike activity. By aggregating neuron responses over time and across groups, the model leverages collective spiking behavior to produce stable and reliable predictions.
...
While the RANC-based Spiking Neural Network in Section \ref{sec:RANC-based-Spiking-Neural-Network} efficiently captures local spatio-temporal features through event-driven computation, it lacks an explicit mechanism to model global dependencies across the input space \cite{RANC}. This limitation is particularly critical for capacitive fingerprint images, where discriminative patterns are often spatially distributed and affected by noise and variability \cite{CapfingerId}. Vision Transformers address this issue via self-attention mechanisms, but their high computational cost limits their applicability in resource-constrained environments \cite{dosovitskiy2021imageworth16x16words}. To overcome these challenges, this work adopts the SpikeFormer architecture, which integrates self-attention into the spiking domain to enable global feature modeling while maintaining energy efficiency \cite{zhou2023spikeformer}.

\subsection{Overview of SpikeFormer Architecture}

The overall architecture of SpikeFormer follows a Transformer-style pipeline adapted to spike-based computation \cite{zhou2023spikeformer}. Given an input spatio-temporal representation $I \in \mathbb{R}^{T \times C \times H \times W}$, where $T$, $C$, $H$, and $W$ denote the number of time steps, channels, height, and width respectively, the model processes the data through a sequence of spike-driven modules to produce the final prediction.
...
\begin{align}
x = \mathrm{SPS}(I), \quad x \in \mathbb{R}^{T \times N \times D}.
\end{align}
...
\begin{align}
\mathrm{RPE} = \mathcal{SN}(\mathrm{BN}(\mathrm{Conv2d}(x))), \quad \mathrm{RPE} \in \mathbb{R}^{T \times N \times D}.
\end{align}
...
The resulting sequence is processed by a stack of $L$ SpikeFormer encoder blocks. Each block consists of a Spiking Self-Attention (SSA) module followed by a spiking multi-layer perceptron (MLP), both equipped with residual connections:
\begin{align}
X'_l &= \mathrm{SSA}(X_{l-1}) + X_{l-1}, \\
X_l &= \mathrm{MLP}(X'_l) + X'_l, \quad l = 1, \dots, L.
\end{align}
...
\begin{figure}[h]
    ...
    \caption{Overview of SpikeFormer architecture. The model consists of a Spiking Patch Splitting (SPS) module for input embedding, followed by a stack of SpikeFormer encoder blocks that utilize Spiking Self-Attention (SSA) and spiking MLPs. Finally, a global average pooling layer and classification head produce the output prediction.}
\end{figure}

\subsection{Spiking Patch Splitting (SPS)} \label{sec:patch_transformer}

The Spiking Patch Splitting (SPS) module is responsible for transforming the input spike-based image representation into a sequence of embedded patches suitable for Transformer processing \cite{zhou2023spikeformer}. Given an input tensor $I \in \mathbb{R}^{T \times C \times H \times W}$, SPS projects the input into a $D$-dimensional spike-form feature space while simultaneously partitioning it into patch tokens.
...
\begin{align}
x = \mathscr{MP}\left(\mathcal{SN}(\mathrm{BN}(\mathrm{Conv2d}(I)))\right),
\end{align}
where $\mathrm{Conv2d}$ denotes a convolutional layer with kernel size $3 \times 3$ and stride 1, $\mathrm{BN}$ represents batch normalization, $\mathcal{SN}$ is the spiking neuron function, and $\mathscr{MP}$ denotes max-pooling for spatial downsampling.

In this work, the spiking neuron function $\mathcal{SN}$ is implemented using a multi-step Leaky Integrate-and-Fire (LIF) model, specifically the \texttt{MultiStepLIFNode} provided by the SpikingJelly \cite{SpikingJelly} framework, which integrates input signals over time and generates binary spike outputs in a vectorized manner.

Similar to the convolutional stem in Vision Transformers \cite{xiao2021early,hassani2021escaping}, the inclusion of convolutional layers introduces inductive bias into the model, improving its ability to capture local spatial structures. Multiple SPS blocks can be stacked to progressively refine feature representations. In such cases, the number of output channels is gradually increased across blocks to match the desired embedding dimension $D$. For example, with a four-block SPS design, the channel dimensions can be configured as $D/8$, $D/4$, $D/2$, and $D$, respectively.
...
\subsection{Spiking Self-Attention Mechanism}

The conventional self-attention mechanism is highly effective in modeling global dependencies but relies on dense floating-point operations and softmax normalization, which are not well-suited for spiking neural networks. To address this limitation, this work adopts the Spiking Self-Attention (SSA) mechanism proposed in SpikeFormer \cite{zhou2023spikeformer}, which reformulates attention into a spike-driven computation paradigm.
...
\begin{align}
Q = \mathcal{SN}_Q(\mathrm{BN}(XW_Q)), \quad 
K = \mathcal{SN}_K(\mathrm{BN}(XW_K)), \quad 
V = \mathcal{SN}_V(\mathrm{BN}(XW_V)),
\end{align}
