...
Spiking feature extraction in the proposed architecture is performed using the \textit{Tea layer}, which serves as the fundamental computational unit of the network. Each \textit{Tea layer} implements a hardware-constrained spiking transformation that approximates neuromorphic computation while remaining trainable using gradient-based methods.

Let $\mathbf{x} \in \{0,1\}^N$ denote the input spike vector after encoding. The \textit{Tea layer} maintains two sets of parameters: a static weight matrix $\mathbf{W} \in \{-1,1\}^{N \times M}$ and a trainable connection matrix $\mathbf{C} \in [0,1]^{N \times M}$. During the forward pass, the connection matrix is discretized into a binary mask:
...
Each input segment is processed independently by a dedicated \textit{Tea layer}, allowing the network to extract localized spiking features in parallel. The resulting output vectors encode the spatio-temporal firing behavior of neurons and serve as high-level representations for subsequent aggregation and classification.
